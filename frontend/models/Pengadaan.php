...
class Pengadaan extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['tanggal_pengadaan', 'no_pengadaan', 'nama_kegiatan', 'keterangan_pengadaan', 'kode_unit_kerja', 'kategori_barangjasa', 'nama_barangjasa', 'spesifikasi_barangjasa', 'jumlah_pengadaan', 'jenis_satuan_pengadaan', 'nama_penyedia', 'waktu_pelaksanaan'], 'required'],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'nama_barangjasa' => 'Nama Barang/Jasa',
    ...
    public function Buatrp(){
        $nilai = Pengadaan::findOne($this->id_pengadaan);
        $angka = $nilai->nilai_kontrak;
        $jadi = "Rp ".number_format($angka,0,',','.').",-";
        return $jadi;
    }

    public function getKodeKegiatan(){
        return $this->hasOne(KodeKegiatan::className(), ['id_kegiatan' => 'kode_kegiatan']);
    }

    public function getTahunPengadaan(){
        $tahun = $this->tanggal_pengadaan;
        return date('Y',strtotime($tahun));
    }
}
